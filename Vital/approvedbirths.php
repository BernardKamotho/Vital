   	    ...
   	    <div class="col-md-12">
             ...
             <i>Filter by;</i>
              ...
              if (empty($_POST['end'])) {
               ...
               $ReadSql = "SELECT * FROM `birthcertificat_app_form` WHERE status = 'Approved'";
               

               if (!empty($_POST) and $_POST['notification_no'] != '') {
                 $ReadSql = "SELECT * FROM `birthcertificat_app_form` WHERE status = 'Approved' and notification_no ='".$_POST['notification_no']."'";
               }


                if (!empty($_POST) and $_POST['applicant_idno'] != '') {
                 $ReadSql = "SELECT * FROM `birthcertificat_app_form` WHERE  status = 'Approved' and applicant_idno ='".$_POST['applicant_idno']."'";
                }

                 if (!empty($_POST) and $_POST['district'] != '') {
                 $ReadSql = "SELECT * FROM `birthcertificat_app_form` WHERE  status = 'Approved' and district ='".$_POST['district']."'";
                }


                if (!empty($_POST) and $_POST['start'] != '' and $_POST['end'] != '') {
                 $ReadSql = "SELECT * FROM `birthcertificat_app_form` WHERE  status = 'Approved' and dob  BETWEEN '".$_POST['start']."' 
                 AND '".$_POST['end']."' ";
                }
               ...
               $res = mysqli_query($connection, $ReadSql); 
                ...
                echo "<table class='table'>";
                    ...
                    while($r = mysqli_fetch_assoc($res)){
                            ...
                            href=rejectbirth.php?id=$id>Reject</span></a></td>";
