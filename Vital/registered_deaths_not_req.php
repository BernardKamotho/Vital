   	    ...
   	    <div class="col-md-12">
             ...
             <i>Filter by;</i>
              ...
              if (empty($_POST['end'])) {
               ...
               $ReadSql = "SELECT * FROM `deathcertificate_form` WHERE status = 'Not Approved' AND user_applied ='Not Applied'";
               

               if (!empty($_POST) and $_POST['deceased_idno'] != '') {
                 $ReadSql = "SELECT * FROM `deathcertificate_form` WHERE status = 'Not Approved' AND user_applied ='Not Applied' and deceased_idno ='".$_POST['deceased_idno']."'";
               }


                if (!empty($_POST) and $_POST['permit_number'] != '') {
                 $ReadSql = "SELECT * FROM `deathcertificate_form` WHERE  status = 'Not Approved' AND user_applied ='Not Applied' and permit_number ='".$_POST['permit_number']."'";
                }

                if (!empty($_POST) and $_POST['start'] != '' and $_POST['end'] != '') {
                 $ReadSql = "SELECT * FROM `deathcertificate_form` WHERE  status = 'Not Approved' AND user_applied ='Not Applied' and dod  BETWEEN '".$_POST['start']."' 
                 AND '".$_POST['end']."' ";
                }
               ...
               $res = mysqli_query($connection, $ReadSql); 
                ...
                echo "Showing $count Records. Certificate not requested by user";
                   ...
                   echo "</tr>";
                            ...
                            echo "<td>".$r['user_applied']."</td>";
                              ...
                              if ($r['status'] =='Approved') {
                                  echo "<td><a href ='https://modcom.co.ke/Vital/".$r['notification_no'].".pdf"."'>View Certificate</a></td>";
                                }


                                if ($r['user_applied'] =='Applied') {
                                   echo "<td><a class = 'btn btn-danger' onClick=\"javascript: return confirm('Are you sure you want to Reject');\"   
                            href=rejectbirth.php?id=$id>Approve</span></a></td>";
                                }
                          echo "</tr>"; 
