   	    ...
   	    <div class="col-md-12">
             ...
             <i>Filter by;</i>
              ...
              if (empty($_POST['end'])) {
               ...
               $ReadSql = "SELECT * FROM `birthcertificat_app_form` WHERE status = 'Not Approved'";
               

               if (!empty($_POST) and $_POST['notification_no'] != '') {
                 $ReadSql = "SELECT * FROM `birthcertificat_app_form` WHERE status = 'Not Approved' and notification_no ='".$_POST['notification_no']."'";
               }


                if (!empty($_POST) and $_POST['applicant_idno'] != '') {
                 $ReadSql = "SELECT * FROM `birthcertificat_app_form` WHERE  status = 'Not Approved' and applicant_idno ='".$_POST['applicant_idno']."'";
                }

                 if (!empty($_POST) and $_POST['district'] != '') {
                 $ReadSql = "SELECT * FROM `birthcertificat_app_form` WHERE  status = 'Not Approved' and district ='".$_POST['district']."'";
                }


                if (!empty($_POST) and $_POST['start'] != '' and $_POST['end'] != '') {
                 $ReadSql = "SELECT * FROM `birthcertificat_app_form` WHERE  status = 'Not Approved' and dob  BETWEEN '".$_POST['start']."' 
                 AND '".$_POST['end']."' ";
                }
